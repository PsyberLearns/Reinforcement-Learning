...
def learn(env,
    ...
    #current q-function network
    cur_q = q_func(obs_t_float, num_actions, scope="cur_q_func", reuse=False)
    cur_q_value = tf.reduce_sum(cur_q * tf.one_hot(act_t_ph, depth=num_actions), axis=1)

    #target q-function network
    ...
    ######
    ''' d("cur_q = {}".format(cur_q))
    d("target_q = {}".format(target_q))

    d("cur_q_value = {}".format(cur_q_value))
    d("target_q_a = {}".format(target_q_value))
    ...
    for t in itertools.count():
        ...
        # YOUR CODE HERE
        latest_index = replay_buffer.store_frame(last_obs)
        ...
        replay_buffer.store_effect(latest_index, action, reward, done)
        if done:
            last_obs = env.reset()
        else:
        ...
        if (t > learning_starts and
            ...
            # YOUR CODE HERE
            #sample a batch of transistions from the replay buffer
            ...
            #keep track of the number of experience replays for target network update later on

            if t % target_update_freq == 0:
                session.run(update_target_fn)
                num_param_updates+=1

            if t % 500 == 0: print("{} timesteps reached in training".format(t))
